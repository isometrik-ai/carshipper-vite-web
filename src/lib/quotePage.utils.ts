...
const quotePageCache: { data?: QuoteResponse; timestamp?: number } = {}; // Cache object

export async function fetchQuotePageData(): Promise<QuoteResponse> {
  const now = Date.now();
  // Cache expires after 5 minutes
  if (quotePageCache.data && quotePageCache.timestamp && now - quotePageCache.timestamp < 300000) {
    return quotePageCache.data;
  }
  ...
  try {
    ...
    // Update cache
    quotePageCache.data = response;
    quotePageCache.timestamp = now;
    return response;
  } catch (error) {
