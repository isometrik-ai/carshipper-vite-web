...
import OpenTransportPageClient from "./OpenTransportPageClient";
...
export async function generateMetadata(): Promise<Metadata> {
  const seo = await fetchStrapiSeoMetadata(STRAPI_API_URL, "/api/open-transport");
  const title = seo?.meta_title || DEFAULT_TITLE;
  const description = seo?.meta_description || DEFAULT_DESCRIPTION;
  const canonical = seo?.canonical_url || `${SITE_URL}/open-transport`;
...
export default function OpenTransportPage() {
  return <OpenTransportPageClient />;
}
