...
export default function OpenTransportPageClient() {
  ...
  const pageData = useMemo(() => {
    ...
    const specificsSection = content.find(
      c => c.__component === "shared.service-cards" && (c as ServiceCards).section_title === "Open Auto Transport Specifics"
    ) as ServiceCards | undefined;
    const benefitsSection = content.find(
      c => c.__component === "shared.service-cards" && (c as ServiceCards).section_title === "Why Choose Our Open Carrier Services?"
    ) as ServiceCards | undefined;
    ...
    <main className="flex-1 pt-20">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              {pageData.hero?.description ? <p className="text-lg md:text-xl text-muted-foreground mb-6">{pageData.hero.description}</p> : null}
                  ...
                  {pageData.hero.trust_indicators.map((indicator) => {
                    ...
                    return (
                      <div key={indicator.id} className={`flex items-center gap-2 ${bgColor} px-4 py-2 rounded-full`}>
                        {IndicatorIcon ? <IndicatorIcon className="w-4 h-4" aria-hidden="true" /> : null}
        ...
        <section className="py-16 md:py-20 bg-muted/30">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              ...
              >
                {pageData.textSection.section_title ? <h2 className="text-2xl font-bold mb-4">{pageData.textSection.section_title}</h2> : null}
        ...
        <section className="py-16 md:py-24 bg-muted/30">
          <div className="container mx-auto px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="text-center mb-12">
              {pageData.specifics.section_title ? <h2 className="text-3xl md:text-4xl font-bold mb-4">{pageData.specifics.section_title}</h2> : null}
              {pageData.specifics.section_subtitle ? <p className="text-lg text-muted-foreground max-w-2xl mx-auto">{pageData.specifics.section_subtitle}</p> : null}
            </motion.div>
            ...
            {pageData.specifics.service_cards && pageData.specifics.service_cards.length > 0 ? (
              <div className="grid md:grid-cols-2 gap-8">
                {pageData.specifics.service_cards.map((feature: any, index: number) => {
                  const FeatureIcon = feature.icon_name ? (getIcon(feature.icon_name) as LucideIcon) : null;
                  return (
                    <motion.div
      ...
      {pageData.benefits ? (
        <section className="py-16 md:py-24">
          <div className="container mx-auto px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="text-center mb-12">
              {pageData.benefits.section_title ? <h2 className="text-3xl md:text-4xl font-bold mb-4">{pageData.benefits.section_title}</h2> : null}
            </motion.div>
            ...
            {pageData.benefits.service_cards && pageData.benefits.service_cards.length > 0 ? (
              <div className="grid md:grid-cols-3 gap-8">
                {pageData.benefits.service_cards.map((benefit: any, index: number) => {
                  const BenefitIcon = benefit.icon_name ? (getIcon(benefit.icon_name) as LucideIcon) : null;
                  return (
                    <motion.div
      ...
      {pageData.safety ? (
        <section className="py-16 md:py-24 bg-secondary/50">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="bg-card rounded-3xl p-8 md:p-12 border border-border">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  {pageData.safety.section_title ? <h2 className="text-2xl md:text-3xl font-bold">{pageData.safety.section_title}</h2> : null}
                </div>
      ...
      {pageData.testimonials && pageData.testimonials.testimonials && pageData.testimonials.testimonials.length > 0 ? (
        <section className="py-16 md:py-24">
          <div className="container mx-auto px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="text-center mb-12">
              {pageData.testimonials.section_title ? <h2 className="text-3xl md:text-4xl font-bold mb-4">{pageData.testimonials.section_title}</h2> : null}
            </motion.div>
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              {pageData.testimonials.testimonials.map((testimonial: any, index: number) => (
                <motion.div key={testimonial.id || index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} className="bg-card p-6 rounded-2xl border border-border">
                  ...
                  {testimonial.title ? <h3 className="text-lg font-semibold mb-3">{testimonial.title}</h3> : null}
                  {testimonial.quote_text ? <p className="text-muted-foreground">{testimonial.quote_text}</p> : null}
                </motion.div>
              ))}
            </div>
          </div>
      ...
      {pageData.faq ? (
        <section className="py-16 md:py-24 bg-muted/30">
          <div className="container mx-auto px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="text-center mb-12">
              {pageData.faq.section_title ? <h2 className="text-3xl md:text-4xl font-bold mb-4">{pageData.faq.section_title}</h2> : null}
            </motion.div>

            {pageData.faq.faq_items && pageData.faq.faq_items.length > 0 ? (
              <div className="max-w-3xl mx-auto space-y-6">
                {pageData.faq.faq_items.map((faq: any, index: number) => (
                  <motion.div key={faq.id || index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} className="bg-card p-6 rounded-xl border border-border">
