...
import DealershipDeliveryPageClient from "./DealershipDeliveryPageClient";
...
async function fetchDealershipDeliverySeo(): Promise<DealershipDeliveryResponse["data"]["seo_metadata"] | null> {
  ...
  try {
    const response = await fetch(`${STRAPI_API_URL}/api/dealership-delivery${seoQuery}`, {
      next: { revalidate: 60 },
    });
    if (!response.ok) return null;
    const payload = (await response.json()) as DealershipDeliveryResponse;
...
export async function generateMetadata(): Promise<Metadata> {
  const seo = await fetchDealershipDeliverySeo();
  const title = seo?.meta_title || "Dealership Delivery | CarShippers AI";
  const description =
    seo?.meta_description ||
    "Reliable dealership vehicle delivery with transparent pricing and trusted carriers.";
  const canonical = seo?.canonical_url || `${SITE_URL}/dealership-delivery`;
...
export default function DealershipDeliveryPage() {
  return <DealershipDeliveryPageClient />;
}
