...
export default function LosAngelesShippingPageClient() {
  ...
  const pageData = useMemo(() => {
    ...
    const routeTableFrom = content.find(
      c => c.__component === "shared.route-table" && (c as RouteTable).section_title?.includes("FROM Los Angeles")
    ) as RouteTable | undefined;
    const routeTableTo = content.find(
      c => c.__component === "shared.route-table" && (c as RouteTable).section_title?.includes("TO Los Angeles")
    ) as RouteTable | undefined;
            ...
            {pageData.neighborhoods.neighborhoods && pageData.neighborhoods.neighborhoods.length > 0 ? (
              <div className="grid md:grid-cols-3 gap-4">
                {pageData.neighborhoods.neighborhoods.map((neighborhood: any) => (
                  <div key={neighborhood.id} className="bg-card p-4 rounded-xl border border-border">
