...
import LosAngelesShippingPageClient from "./LosAngelesShippingPageClient";
...
const toAbsoluteUrl = (url?: string | null): string | undefined => {
  ...
  if (url.startsWith("/") && !url.includes("//")) return `${SITE_URL}${url}`;
  return undefined;
};
...
export async function generateMetadata(): Promise<Metadata> {
  const seo = await fetchStrapiSeoMetadata(STRAPI_API_URL, "/api/los-angeles-car-shipping");
  const title = seo?.meta_title || DEFAULT_TITLE;
  const description = seo?.meta_description || DEFAULT_DESCRIPTION;
  const canonical = seo?.canonical_url || `${SITE_URL}/los-angeles-car-shipping`;
...
export default function LosAngelesShippingPage() {
  return <LosAngelesShippingPageClient />;
}
