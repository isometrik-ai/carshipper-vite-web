...
async function fetchFAQSeo(): Promise<FAQResponse["data"]["seo_metadata"] | null> {
  ...
  try {
    const response = await fetch(`${STRAPI_API_URL}/api/faq${seoQuery}`, {
      next: { revalidate: 60 },
    });
...
export async function generateMetadata(): Promise<Metadata> {
  const seo = await fetchFAQSeo();
  const title = seo?.meta_title || "FAQ | CarShippers AI";
  const description =
    seo?.meta_description ||
    "Find answers to common car shipping questions, pricing, timing, and transport options.";
  const canonical = seo?.canonical_url || `${SITE_URL}/faq`;
