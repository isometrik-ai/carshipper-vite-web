...
import HeavyHaulingPageClient from "./HeavyHaulingPageClient";
...
export async function generateMetadata(): Promise<Metadata> {
  const seo = await fetchStrapiSeoMetadata(STRAPI_API_URL, "/api/heavy-hauling");
  const title = seo?.meta_title || DEFAULT_TITLE;
  const description = seo?.meta_description || DEFAULT_DESCRIPTION;
  const canonical = seo?.canonical_url || `${SITE_URL}/heavy-hauling`;
...
export default function HeavyHaulingPage() {
  return <HeavyHaulingPageClient />;
}
