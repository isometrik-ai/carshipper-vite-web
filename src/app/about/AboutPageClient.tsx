      ...
      <section className="py-16 md:py-24 bg-gradient-to-b from-secondary/50 to-background" aria-labelledby="about-hero-heading">
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-muted-foreground mb-8">{pageData.hero?.description || ""}</p>
          </motion.div>
        </div>
      </section>
        ...
        <section className="py-12 bg-primary" aria-label="Statistics">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8" role="list">
              {pageData.stats.map((stat: { id?: number; label?: string; value?: string }, index: number) => (
                <motion.div
      ...
      {pageData.values ? (
        <section className="py-16 md:py-24 bg-muted/30" aria-labelledby="our-values-heading">
          <div className="container mx-auto px-4">
            ...
            {pageData.values?.steps?.length > 0 ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8" role="list" aria-label="Company values">
                {pageData.values.steps.map((value: { id?: number; title?: string; description?: string; icon_name?: string }, index: number) => {
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              ...
              >
                ...
                {pageData.why30Minutes?.intro_text ? <p className="text-muted-foreground mb-6">{pageData.why30Minutes.intro_text}</p> : null}
                {pageData.why30Minutes?.bullet_points?.length > 0 ? (
                  <ul className="space-y-4">
                    {pageData.why30Minutes.bullet_points.map((point: { id?: number; text?: string }) => (
                ...
                {pageData.why30Minutes?.closing_text ? <p className="text-muted-foreground mt-6">{pageData.why30Minutes.closing_text}</p> : null}
              </motion.div>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
              ...
              {pageData.cta?.description ? (
                <p className="text-xl text-primary-foreground/80 mb-8 max-w-2xl mx-auto">{pageData.cta.description}</p>
              ) : null}
              {pageData.cta?.primary_button ? (
                <Button
                  ...
                  onClick={() => pageData.cta?.primary_button?.button_link && (window.location.href = pageData.cta.primary_button.button_link)}
                >
