  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-muted/30">
      <div className="max-w-md w-full rounded-lg border bg-card p-6 text-center shadow-sm">
        {status === "idle" ? <p className="text-muted-foreground">Purging cache…</p> : null}
        {status === "success" ? (
          <>
        ...
        {status === "forbidden" ? <p className="text-destructive">Invalid or missing token.</p> : null}
        {status === "error" ? <p className="text-destructive">Failed to clear cache.</p> : null}
        <Link
          href="/"
...
export default function PurgeCachePageClient() {
  return (
