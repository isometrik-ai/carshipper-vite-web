...
const fetchQuoteForm = async (): Promise<QuoteFormResponse> => {
  const response = await fetch(
    `${STRAPI_API_URL}/quote-form?populate[form_config][populate][steps][populate][fields][populate]=*&populate[form_config][populate][timeframe_options][populate]=*&populate[form_config][populate][transport_type_options][populate]=*&populate[form_config][populate][button_texts][populate]=*&populate[form_config][populate][running_status_options][populate]=*&populate[form_config][populate][vehicle_field_config][populate]=*&populate[form_config][populate][vehicle_data][populate][models][populate]=*`
  );
